...
Search indexes are projections, not authorities. Index documents must include tenant key, resource type, resource ID, authorization scope hints, deletion state, source version, and indexed timestamp. Search results must be reauthorized against source data as required by [../06-tenant-isolation.md](../06-tenant-isolation.md).
