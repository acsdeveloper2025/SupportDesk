...
- Asset Management (Module 3): `asset.type.read/create/update/delete`, `asset.category.read/create/update/delete`, `asset.location.read/create/update/delete`, `asset.read/create/update/delete/transition/assign/unassign`, `asset.relationship.create/delete`, `asset.history.read`, `asset.ticket.link/unlink`, `asset.kb.link/unlink`, `asset.attachment.create/read/delete`.
- Enterprise Reports & Analytics (Module 4): `report.read`, `report.ticket.read`, `report.sla.read`, `report.workflow.read`, `report.asset.read`, `report.catalog.read`, `report.kb.read`, `report.agent.read`, `report.saved.read/create/update/delete`, `report.schedule.read/create/update/delete`, `report.export.create/read/download`.
- Enterprise Administration & Platform Management (Module 5): `admin.global.read/update`, `admin.tenant.read/create/update/lifecycle`, `admin.user.read/invite/update/lockout/session`, `admin.role.manage`, `admin.permission.read`, `admin.workflow.manage/retry`, `admin.outbox.manage/replay`, `admin.sla.manage`, `admin.notification.manage`, `admin.audit.read`, `admin.security.read`, `admin.health.read`, `admin.diagnostics.read`, `admin.feature_flag.manage`.

Platform-only keys (granted to the `platform_admin` elevation role only, never to `tenant_admin`): `admin.tenant.create`, `admin.tenant.lifecycle`, `admin.global.read`, `admin.global.update`, `admin.feature_flag.manage`. These guard platform-global resources (global settings, global feature flags, platform-wide maintenance windows, tenant provisioning/lifecycle). The `tenant_admin` role is seeded with the remaining Module 5 keys for tenant-scoped operations (workflows, outbox replay, SLA, notifications, users, roles, audit, health).
